...
RSpec.describe 'Garden Show Page' do
  before :each do
    ...
    @plant7 = Plant.create!(name: "Carrot", description: 'cool', days_to_harvest: 5)
    @pp1 = PlotPlant.create!(plot_id: @plot1.id, plant_id: @plant1.id)
    @pp2 = PlotPlant.create!(plot_id: @plot2.id, plant_id: @plant1.id)
  ...
  describe 'user story 3' do
    it 'lists all plants that have less than 100 days harvest time for garden' do
    ...
    describe 'user story 4' do
      it 'sorted by most number of plants' do
        visit garden_path(@garden1)

        expect(@plant1.name).to appear_before(@plant7.name)
      end
    end
  end
end
